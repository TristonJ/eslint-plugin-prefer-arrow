...
const isStandaloneDeclaration = node => node.type === 'FunctionDeclaration' && (!node.parent || node.parent.type === 'Program');
...
const inspectNode = (node, context) => {
  ...
  if(opts.allowStandaloneDeclarations && isStandaloneDeclaration(node)) return;
